...
function downloadFromUrlLink(fileName, url) {
	// Create an invisible A element
	const a = document.createElement("a");
	a.style.display = "none";
	document.body.appendChild(a);
...
var setCaretPosition = function(elem, chars) {
	if (chars >= 0) {
		let selection = window.getSelection();

		let range = createRange(elem, {
			count: chars
		});
...
export {
	isSymbol, get, eraseCookieFromAllPaths, downloadFromUrl, downloadFromUrlLink, download, getFontAwesomeIconFromMIME, getCaretPosition, setCaretPosition
};